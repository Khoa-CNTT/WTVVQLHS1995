            ...
            {currentUsers.length > 0 ? (
              currentUsers.map((user, index) => (
                <tr key={user.id} className={index % 2 === 0 ? styles.evenRow : styles.oddRow}>
                  ...
                  <td>{user.description || 'Chưa có'}</td>
                  <td>{user.role}</td>
                  <td>{user.is_verified ? 'Đã xác minh' : 'Chưa xác minh'}</td>
                  <td>
                    <span className={user.is_locked ? styles.statusLocked : styles.statusActive}>
                      {user.is_locked ? 'Đã khóa' : 'Hoạt động'}
                    </span>
                  </td>
                  <td>{user.last_login ? new Date(user.last_login).toLocaleString() : 'Chưa đăng nhập'}</td>
